...
usage() {                                      # Function: Print a help message.
	echo "Usage: $0 [ -i INPUT (ab1, fasta, fastq) ]" 1>&2
}
...
if [[ ! $@ =~ ^\-i+ ]]; then
	die "Requires the option -i (input type)."
fi
while getopts "i:" options; do              # Loop: Get the next option;
	case "${options}" in                         #
		i)                                         # If the option is t,
			INPUT=${OPTARG}                          # Set $TIMES to specified value.
			if [ $INPUT = "ab1" ] ; then   # if $TIMES not a whole number:
				echo "Input type ab1 ..."
			elif [ $INPUT = "fasta" ] ; then   # if $TIMES not a whole number:
				echo "Input type fasta ..."
			elif [ $INPUT = "fastq" ] ; then   # if $TIMES not a whole number:
